...
# Set bot commands
async def set_commands(robot: Bot):
    commands = [
...
Hello! I’m your friendly YouTube Audio Downloader Bot. My job is to help you effortlessly save music from YouTube as audio files. 
...
async def echo_message(message: types.Message):
    if message.text.startswith("https://www.youtube.com/watch?") or message.text.startswith("https://youtu.be/"):
        ...
        if os.path.getsize(file_location) > 52428800:
            ...
            await upload_to_telegram(file_location, thumbnail_location, file_caption)

        await send_photo(message.chat.id, thumbnail_location,
...
async def upload_to_telegram(file_path, thumbnail_path, file_caption):
    ...
    try:
        ...
        await bot.send_audio(chat_id=channel_id, audio=audio_file, caption=f"🔉 **{file_caption}**",
                             thumbnail=thumbnail_file, parse_mode="Markdown")
